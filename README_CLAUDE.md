...
- 虽本文用中文书写，但用户前端展示内容需使用英文。

## 变量

### 本地存储

保存到以 `. + {变量名}` 命名的无扩展名文本文件。有此文件时，优先从本地读取。

| 变量名      | 文件名       | 类型  | 值        | 说明                        |
| ----------- | ------------ | ----- | --------- | --------------------------- |
| `device_id` | `.device_id` | `str` | 随机 UUID | 设备唯一 ID（鉴权请求头用） |
...
环境变量或存储于 `.env`。有默认值的为可选配置项，允许此环境变量为空或不存在；`{}` 为占位符，用于程序自动生成。
...
| `HALF_REPLAY_YEAR_FORMAT`       | `str` (`strftime`) | `'%Y'`                       | 半年回忆年格式           |
| `HALF_REPLAY_TITLE_TEMPLATE`    | `str`              | `'Replay {year} H{half}'`    | 半年回忆标题模板         |
| `IS_QUARTER_REPLAY_DISABLED`    | `bool`             | `False`                      | 是否禁用季度回忆生成     |
| `QUARTER_REPLAY_LIMIT`          | `int`              | `25`                         | 季度回忆数量限制         |
| `QUARTER_REPLAY_YEAR_FORMAT`    | `str` (`strftime`) | `'%Y'`                       | 季度回忆年格式           |
| `QUARTER_REPLAY_TITLE_TEMPLATE` | `str`              | `'Replay {year} Q{quarter}'` | 季度回忆标题模板         |
...
此后所有请求，需携带 `MediaBrowser Client="{MEDIA_BROWSER_CLIENT}", Device="{DEVICE}", DeviceId="{device_id}", Version="{VERSION}", Token="{access_token}"` (`snake` 写法的 `AccessToken`) 鉴权请求头。
...
3. 通过 `Name`、`Album`、`AlbumArtist`（**注意**！是专辑艺人而不是艺人）获取项目 `Id`。
    - 错误处理：移除没有找到的项目，并控制台警告。允许数量减少或少于 {LIMIT}。
4. 输出每个周期的项目 `id` 列表。
...
2. 处理每一个周期列表：
    1. 构建周期播放列表请求体。示例：
        ...
        ```
    2. 通过用户播放列表 API 响应中的 `Name` 字段，查找待处理的播放列表名称是否存在。
        - 如果存在，带请求体更新播放列表。
            - Curl 示例：
                ```bash
                curl -X 'POST' \
...
| 占位符      | 类型  | 取值范围           | 示例                                                             | 说明                                     |
| ----------- | ----- | ------------------ | ---------------------------------------------------------------- | ---------------------------------------- |
| `{year}`    | `int` | `r'\d{2}(\d{2})?'` | `2025` (`YEAR_FORMAT` == `'%Y'`)、`25` (`YEAR_FORMAT` == `'%y'`) | 各周期使用相同的年占位符，但格式独立配置 |
| `{half}`    | `int` | in `[1, 2]`        |
| `{quarter}` | `int` | in `[1, 2, 3, 4]`  |

### 名称配置示例

区别于默认值，这里展示了一套名称配置搭配。示例中所有配置应能在同一组中出现。

- 年：
    - `YEAR_REPLAY_YEAR_FORMAT` = `'%Y'`
...
- 半年：
    ...
    - 播放列表名称：
        ...
        ```
- 季度：
    - `QUARTER_REPLAY_YEAR_FORMAT` = `'%Y'`
    - `QUARTER_REPLAY_TITLE_TEMPLATE` = `'Replay {year} Q{quarter}'`
    - 播放列表名称：
        ```text
        Replay Q3 2025
        Replay Q4 2025
